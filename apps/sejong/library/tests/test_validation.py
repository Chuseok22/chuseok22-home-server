...
def test_validate_attendee_count_passes_within_range(seat_cnt, attendee_count, expected):
    assert validate_attendee_count(seat_cnt, attendee_count) is expected


def test_validate_attendee_count_fails_below_half():
    message = validate_attendee_count(6, 2)
    assert message == '정원(6명)의 절반 이상인 최소 3명, 최대 6명이 필요합니다.'


def test_validate_attendee_count_fails_above_capacity():
    message = validate_attendee_count(6, 7)
    assert message == '정원(6명)의 절반 이상인 최소 3명, 최대 6명이 필요합니다.'


def test_validate_attendee_count_odd_capacity_rounds_up_minimum():
