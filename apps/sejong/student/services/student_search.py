...
class StudentSearchService:
    ...
    def _search(self, category: str, search_value: str) -> list[StudentInfo] | None:
        auth = SejongClassicAuthService()
        ...
        try:
            ...
            logger.error('학생 조회 요청 실패 (category=%s, value=%s): %s', category, search_value, e)
            return None  # 장애 — 뷰에서 503 반환
        ...
        for user in data.get('userList', []):
            try:
                results.append(StudentInfo(
                    ...
                    email=user.get('email', ''),  # email 누락 방어
