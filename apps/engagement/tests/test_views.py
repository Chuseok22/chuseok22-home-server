...
@pytest.mark.django_db
def test_로그인_사용자는_장소에_댓글을_작성할_수_있다() -> None:
    user = User.objects.create_user(username='reader')
    ...
    client = Client()
    client.force_login(user)
    url = reverse('engagement:comment-create', kwargs={'app_label': content_type.app_label, 'model': content_type.model, 'object_id': place.pk})
    response = client.post(url, {'body': '여기 진짜 맛있어요'})

    assert response.status_code == 200
    assert Comment.objects.filter(content_type=content_type, object_id=place.pk, body='여기 진짜 맛있어요').exists()


@pytest.mark.django_db
def test_로그인_사용자는_장소에_좋아요를_누를_수_있다() -> None:
    user = User.objects.create_user(username='reader')
    place = Place.objects.create(
        name='몽탄', latitude=Decimal('37.540000'), longitude=Decimal('127.070000'),
    )
    ...
    client = Client()
    client.force_login(user)
    url = reverse('engagement:like-toggle', kwargs={'app_label': content_type.app_label, 'model': content_type.model, 'object_id': place.pk})
    response = client.post(url)

    assert response.status_code == 200
    assert Like.objects.filter(content_type=content_type, object_id=place.pk, user=user).exists()
