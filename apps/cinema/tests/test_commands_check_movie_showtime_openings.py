...
def test_이미_알림_보낸_날짜가_있으면_그_다음날부터_확인한다() -> None:
    ...
    today = timezone.localdate()
    OpenedShowDate.objects.create(tracked_movie=tracked, show_date=today + timedelta(days=5), showtimes=[])
    command = Command()
...
def test_14일_horizon을_넘는_날짜는_후보에서_제외된다() -> None:
    ...
    today = timezone.localdate()
    OpenedShowDate.objects.create(tracked_movie=tracked, show_date=today + timedelta(days=13), showtimes=[])
    command = Command()
