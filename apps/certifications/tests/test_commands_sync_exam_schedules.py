...
@pytest.mark.django_db
def test_상시_접수_자격증은_건너뛴다() -> None:
    # crawler_type이 manual이 아니어도(데이터 입력 실수 등으로) is_always_open=True면
    # ExamSchedule을 생성하지 않는다는 모델 계약을 지켜야 한다.
    CertificationDefinition.objects.create(
        name='CCNA', issuer='Cisco', category=CertificationDefinition.Category.IT_PRIVATE,
        crawler_type='hrdkorea_api', crawler_source_id='9999', is_always_open=True,
    )
...
@pytest.mark.django_db
def test_접수_시작일이_바뀌면_접수시작_알림_플래그가_초기화된다() -> None:
    cert = CertificationDefinition.objects.create(
        name='정보처리기사', issuer='한국산업인력공단',
        category=CertificationDefinition.Category.NATIONAL_TECH, crawler_type='hrdkorea_api',
        crawler_source_id='1320',
    ...
    ExamSchedule.objects.create(
        certification=cert, round_name='2026년 1회 필기',
        registration_start=date(2026, 1, 5), registration_end=date(2026, 1, 9),
        registration_open_notified=True, registration_deadline_notified=True,
    )
    mock_crawler = type('MockCrawler', (), {
        'crawl': lambda self, source_id: [
            ExamRoundItem(
                round_name='2026년 1회 필기',
                registration_start=date(2026, 1, 6), registration_end=date(2026, 1, 9),  # 시작일만 변경
                exam_date=None, result_announcement_date=None, source_url='',
            ),
        ],
    })()
...
def test_접수_마감일이_바뀌면_접수마감_알림_플래그가_초기화된다() -> None:
    ...
    mock_crawler = type('MockCrawler', (), {
        'crawl': lambda self, source_id: [
            ExamRoundItem(
                round_name='2026년 1회 필기',
                registration_start=date(2026, 1, 5), registration_end=date(2026, 1, 12),  # 마감일만 변경
                exam_date=None, result_announcement_date=None, source_url='',
            ),
        ],
    })()
    ...
    schedule = ExamSchedule.objects.get(certification=cert, round_name='2026년 1회 필기')
    assert schedule.registration_end == date(2026, 1, 12)
    assert schedule.registration_open_notified is True
