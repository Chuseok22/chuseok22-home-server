...
- 검증 방식 우선순위: 단위/통합 테스트(pytest) + 실제 동작 확인(개발 환경 `check`/`migrate` 직접 실행, 외부 연동 실제 호출)을 함께 사용한다. 외부 API 연동처럼 pytest로 검증하기 어려운 부분은, `check`/`migrate` 외의 management command라면 사용자가 실행한 결과로 보완한다.
